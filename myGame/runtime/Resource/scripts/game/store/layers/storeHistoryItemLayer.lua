...
function StoreHistoryItemLayer:ctor(data)
    ...
    local nameLabel = new(Text, s, 100, 40, kAlignCenter, nil, 18, 255, 255, 255)
    ...
    local timeLabel = new(Text, s, 100, 40, kAlignCenter, nil, 18, 255, 255, 255)
    timeLabel:setAlign(kAlignLeft)
    timeLabel:setPos(320, 10)
    self:addChild(timeLabel)

     -- 订单状态
    local s = bm.LangUtil.getText("STORE", "RECORD_STATUS")[checkint(data.status)] or ""
    local statusLabel = new(Text, s, 100, 40, kAlignCenter, nil, 18, 255, 255, 255)
    statusLabel:setAlign(kAlignLeft)
    statusLabel:setPos(550, 10)
