...
function DataProxy:cacheData(key)
    ...
    if data then
        if not self.withProxy_ then            
            ...
            nk.updateFunctions.cacheTable(key, cacheData)            
        else
...
function DataProxy:getData(key)
    -- dump(self.dataPool_, "selfdadataPool_selfdadataPool_selfdadataPool_")
    if self.dataPool_[key] then
        -- print("11111111111111111111111111 key = ", key)
        return self.dataPool_[key]
    else
        -- print("222222222222222222222222222 key = ", key)
        local cacheData = nk.updateFunctions.cacheTable(key)
...
function DataProxy:removePropertyObserver(key, property, handleToRemove)
    ...
    if propertyHandler[key] and propertyHandler[key][property] and propertyHandler[key][property][handleToRemove] then
...
-- 清理一个data，保留handler
function DataProxy:clearData(key, noNeedHandler)
