...
function StorePayTypeItemLayer:ctor(data, index)
    ...
    if data.discount then
        local discount = new(Image,kImageMap.store_sale)
        discount:setPos(175,0)
        self:addChild(discount)

        local text_dis = new(Text,"+" .. data.discount .. "%",50,20,kAlignCenter,nil,15,255,255,255)
        text_dis:setPos(2,0)
        discount:addChild(text_dis)
    end
