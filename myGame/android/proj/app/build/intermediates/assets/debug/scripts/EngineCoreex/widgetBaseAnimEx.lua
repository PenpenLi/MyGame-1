...
function ActionUnitManager:update(dt)
    s_curSt = dt
    s_isUpdating = true
    local list = self.actionUnitList
...
Action.Rotate = function(args)
    ...
    action.setTarget = function(target)
        ...
        if offset and offset ~= 0 then
            ...
            actionUnit.setValue = function(value)
                FwLog(target.m_drawingID .. " rotation = " .. value .. "s_curSt = " .. s_curSt)
...
function WidgetBase:___updateMatrix()
    ...
    local anchorPixelX = w * anchorX * scaleX * s_systemScale
    local anchorPixelY = h * anchorY * scaleY * s_systemScale
