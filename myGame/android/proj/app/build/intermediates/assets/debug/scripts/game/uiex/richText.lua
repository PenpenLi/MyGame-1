...
local utf8_sub_by_width = function(str, width, fontSize, force)
	local ret = "";
    ...
    while i <= n do
        ...
        local last = i
        local lastStep = 0
        if cp >= 0xF0 then -- 4个字节
        	lastStep = 4
            i = i + lastStep;
            cur_width = cur_width + fontSize;
            morebytecnt = morebytecnt + 1
        elseif cp >= 0xE0 then-- 3个字节
        	lastStep = 3
            i = i + lastStep;
            cur_width = cur_width + fontSize;
            morebytecnt = morebytecnt + 1
        elseif cp >= 0xC0 then-- 2个字节
        	lastStep = 2
            i = i + lastStep;
            cur_width = cur_width + fontSize/2;
            onebytecnt = onebytecnt + 1
        else
        	lastStep = 1
            i = i + lastStep;-- 1个字节
            cur_width = cur_width + fontSize/2;
        ...
        if not e and cur_width > width then
            if onebytecnt > morebytecnt and lastStep <= 2 then
            	if not force then
            		e = lastSpace - 1
            	else
            		e = last - 1 -- leave one space for '-'
            	end
            else
            	e = last - 1
            end
    ...
    e = math.max(e or n, 0)
    ret = string.sub(str, 1, e)
    return ret, cur_width
end
...
RichText.__splitText = function(self, index, cur_line_width, isImage)
	...
	if not isImage then
		...
		-- local sub_str1 = utf8_sub(str, 1, math.floor(self.m_width/self.m_fontSize)-extra_char)
		local sub_str = utf8_sub_by_width(str, self.m_width - cur_line_width, self.m_fontSize)
		...
		-- end
		-- FwLog(">>>>>>>>>>>>>>> sub_str = " .. sub_str)
		local isForce
		if sub_str == "" and cur_line_width == 0 then -- 行首没截断成功，则需要强制截断，否则死循环
			isForce = true
			sub_str = utf8_sub_by_width(str, self.m_width - cur_line_width, self.m_fontSize, true)
		end
		-- 注意！！string.find不接受函数作为参数
		-- FwLog("sub_str = " .. sub_str)
		local temp = SpecialCharSafe2Str(sub_str)
		-- FwLog("temp = " .. temp)
		local s, e = string.find(str, temp)
		local left_str = string.sub(str, e + 1)
		-- FwLog("left_str = " .. left_str)
		-- FwLog(">>>>>>>>>>>>>>> left_str = " .. left_str)
		if string.sub(left_str, 1, 1) == " " then left_str = string.sub(left_str, 2) end -- 处理掉第一个空格
		if isForce then
			self.m_richViews[index]:setText(sub_str .. "-", 0)
		else
			self.m_richViews[index]:setText(sub_str, 0)
		end
		...
		-- 	return self:__splitText(index, cur_line_width, extra_char+ex, false, left_str)
		-- end
		-- self.m_callCount = 0
		return left_str
	else
...
RichText.__preCreate = function(self, lineIndex, index, txt)
	...
	if isOverEdge > 0 and self.m_bAutoNewLine then
		local left_str = self:__splitText(index, curLineWidth, self.m_richCtrl.imageId ~= nil)
		if left_str ~= txt then
			table.insert(self.m_strSequence, 1, {ctrl="l", text=left_str})
		else
			FwLog("Warning: RichText.__preCreate")
		end
	end
	return true
end
...
RichText.analyzeText = function(self, str)
	...
	-- FwLog("final str sequence" .. json.encode(self.m_strSequence))
	self:createFromStr()
end
