...
function Sprite:ctor(imgName,imgPinMap)
	...
	assert(self:getWidget(), "Sprite:ctor with " .. (imgName or "null") .. " fail")
	...
	if info[1] == 2 then
		...
		if isMaskLayer then
			if info[3] ~= 0 then
				...
				if node then
					local mask = self:__getSpriteForMask(info[3])
					node:__setMask(mask)
				else
					node = new(Mask,info[2],info[6])
					local mask = self:__getSpriteForMask(info[3])
					node:__setMask(mask)
...
function MC:__getSpriteForMask(cid)
	if self.m_spriteMasks[cid] then return self.m_spriteMasks[cid] end
	local sp = self.m_mainBody:__getNodeByCid(cid)
	self.m_spriteMasks[cid] = sp
	return sp
end
...
function MC:dtor()
	...
	if self.m_maskDepthMap then
		for k,v in pairs(self.m_maskDepthMap) do
			delete(v)
		end
		self.m_maskDepthMap = nil
	end
	if self.m_spriteMasks then
		for k, v in pairs(self.m_spriteMasks) do
			delete(v)
		end
		self.m_spriteMasks = nil
	end
end
-----------------------------------


function Mask:ctor(maskDepth,clipDepth)
...
function Mask:__setMask(mask)
	self.m_maskLayer:removeAllChildren(false)
...
function Mask:__addContent(sprite,depth)
    ...
    --self.m_contentLayer.cache = true
...
function Swf:ctor(swfInfo,pinMap)
	...
	self.m_imgCache = nil
    self:getWidget().shader = colorOffset
end
...
function Swf:play(frame,isKeep,repeatCount,delay,autoClean)
	self:__resetSprite()
