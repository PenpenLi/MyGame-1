...
AnimBase.setEvent = function(self, obj, func)
	local ret = anim_set_event(self.m_animID,kTrue,self,self.onEvent);
...
WidgetBase.dtor = function(self)
	...
	if self.m_parent then
		WidgetBase.removeChild(self.m_parent, self, false); -- 不能cleanup，否则死循环
		-- self.m_parent:removeChild(self, false, nil); -- 不能cleanup，否则死循环
	end
...
WidgetBase.setVisible = function(self, visible)
    ...
    local ret = drawing_set_visible(self.m_drawingID,self.m_visible and kTrue or kFalse);
    -- if ret ~= 0 then FwLog(debug.traceback("WidgetBase.setVisible")) end
end
...
DrawingBase.doRemoveProp = function(self, sequence)
	if self.m_props[sequence] then
...
DrawingBase.addPropTranslate = function(self, sequence, animType, duration, delay, startX, endX, startY, endY)
	...
	Log.printInfo("startX=", startX .. endX)
...
ResText.ctor = function(self, str, width, height, align, fontName, fontSize, r, g, b, multiLines)
    ...
    -- self.m_font = "Microsoft YaHei Light";
    -- self.m_font = "Microsoft YaHei";
    -- self.m_font = "Droid Sans Fallback";
...
StateMachine.run = function(self)
  local newState = self.m_states[#self.m_states]
  StateMachine.runState(self, newState);

  local newStateObj = newState.stateObj;
  local style = self.m_style or newStateObj.m_style
  if self.m_lastState and style and self.m_styleFuncMap[style] then
    self.m_styleFuncMap[style](newState, self.m_lastState, {releaseLastState = self.m_releaseLastState, 
    	isNewStatePopup = self.m_isNewStatePopup}, self, StateMachine.onSwitchEnd);
  else
    StateMachine.onSwitchEnd(self, newState, self.m_lastState, {releaseLastState = self.m_releaseLastState, 
    	isNewStatePopup = self.m_isNewStatePopup});
  end
  self.m_lastState = nil;
  self.m_releaseLastState = false;
end
...
--
-- @param self 调用者对象。
StateMachine.onSwitchEnd = function(self, newState, lastState, info)
  if lastState then
    if info.releaseLastState then
      StateMachine.cleanState(self, lastState);
    elseif info.isNewStatePopup then

    else
      lastState.stateObj:stop();
    end
  end

  local newStateObj = self.m_states[#self.m_states].stateObj;
  if newStateObj == newState.stateObj then
  	if type(newStateObj.start) == "function" then
  		newStateObj:start()
  	end
  	newStateObj:resume();
  end
...
StateMachine.cleanState = function(self, state)
  ...
  delete(obj);
  if obj.__after_delete then obj:__after_delete() end
end
