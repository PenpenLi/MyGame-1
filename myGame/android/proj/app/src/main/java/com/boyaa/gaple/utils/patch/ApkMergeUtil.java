...
import com.boyaa.engine.patchupdate.PatchUpdate;
 ...
 */
public class ApkMergeUtil implements Runnable{
    ...
    public void run() {
        ...
        do {
            ...
            if (lastApk.exists()) {
                // 存在，验证md5
                if (MD5Util.verify(newApkPath, newApkMD5)) {
                    // md5一致，说明确实合过，不用再合了
                    result = kResultSuccess;
            ...
            //md5验证patch
            if(patchMD5 != "" && !MD5Util.verify(patchPath, patchMD5)){
                result = kResultError;
                break;
            }

            //3.执行命令打新包
            result=PatchUpdateUtil.bspatchUpdate2(sourceApkDir, newApkPath, patchPath);
            ...
            //md5验证newApk
            if(newApkMD5 != "" && !MD5Util.verify(newApkPath, newApkMD5)){
                result = kResultError;
                break;
            }
        ...
        AppActivity.getInstance().runOnLuaThread(new Runnable()
        {
            ...
            public void run() {
                ...
                Dict.setString(kstrDictName, kpatchPath, patchPath);
                Dict.setString(kstrDictName, knewApkPath, newApkPath);
