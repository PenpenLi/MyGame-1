...
public class Downloader {
    ...
    private Map<Integer, DownloadedTaskEntity> mCacheDownloadRecords = new ConcurrentHashMap<Integer, DownloadedTaskEntity>();
    ...
    public Downloader(Context context, String downloadUrl,
                      File saveFile, int threadNum,
                      DownloadListener downloadListener) {
        this(context, downloadUrl, DownloaderConstants.DEFAULT_TIMEOUT,
                DownloaderConstants.DEFAULT_CALLBACK_PERIOD, saveFile,
                threadNum, downloadListener);
    }

    public Downloader(Context context, String downloadUrl,
                      int timeoutMillis, int callbackPeriodMillis, File saveFile,
                      int threadNum, DownloadListener downloadListener) {
        this.mContext = context;
        this.mDownloadUrlStr = downloadUrl;
        this.mLogDao = new DownloadLogDao(this.mContext);
    ...
    public void startDownload() {
        ...
        try {
            ...
            if (conn.getResponseCode() == HttpURLConnection.HTTP_OK) {
                ...
                if (fileSize <= 0) {
                    onDownloadFailed(DownloadListener.ERROR_UNKNOWN_FILE_SIZE);
                    return;
                }

                this.setSourceFileSizeAndCallback(fileSize);
                List<DownloadedTaskEntity> recordList = mLogDao.getDownloadedRecord(mDownloadUrlStr);

                if (recordList.size() > 0) {
                    for(DownloadedTaskEntity entity: recordList) {
                ...
                onDownloadFailed(DownloadListener.ERROR_SERVER_NO_RESPONSE);
            }
        } catch (MalformedURLException e) {
            onDownloadFailed(DownloadListener.ERROR_INCORRECT_URL);
        } catch (ProtocolException e) {
            onDownloadFailed(DownloadListener.ERROR_PROTOCOL_EXCEPTION);
        } catch (IOException e) {
            onDownloadFailed();
        } catch (Exception e) {
            onDownloadFailed();
    ...
    private void countDownloadedSize(Map<Integer, DownloadedTaskEntity> map) {
    ...
    private void distributeTasksAndThread(URL url) {
        for (int i = 0; i < this.mThreads.length; i++) {// 开启线程进行下载
            ...
            if (downloadedSize < this.taskBlockSizePerThread
                ...
                this.mThreads[i] = new DownloadThread(
                        this,
                        url,
                        mSaveFile,
                        taskBlockSizePerThread,
                        downloadedSize,
                        DownloaderConstants.DEFAULT_BUFFER_SIZE_PERTHREAD,
    ...
    private void onDownloadFailed() {
        this.onDownloadFailed(DownloadListener.ERROR_UNKNOWN_ERROR);
    }
    ...
    protected void updateCacheDownloadRecord(int threadId, int downloadedSize) {
        DownloadedTaskEntity entity = this.mCacheDownloadRecords.get(threadId);
